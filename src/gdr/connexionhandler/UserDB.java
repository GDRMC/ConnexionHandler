...
import gdr.connexionhandler.security.SHA256Hash;
import java.io.File;
...
public class UserDB {
    ...
    UserDB(File file) throws FileNotFoundException, IOException, NoSuchAlgorithmException{ this.users=new ArrayList(); readDB(new FileReader(file)); }
    
    public boolean readDB(FileReader fic) throws IOException, NoSuchAlgorithmException
    {
        SHA256Hash h = new SHA256Hash();
        boolean end = false;
	...
	if (entree.ttype ==  StreamTokenizer.TT_EOF){ return false ; }
        while(entree.ttype != StreamTokenizer.TT_EOF){
            ...
            if("!".equals(bufferU)){
                ...
                addUserToDB(bufferU,bufferP,h.getHashFromSHA(bufferP));
            }
        }
    ...
    public void addUserToDB(String bufferU, String bufferP, String hash) throws NoSuchAlgorithmException{
        this.users.add(new User(bufferU,bufferP,hash));
    ...
    public String getUserHash(int n){
        return this.users.get(n).getPassword();
    }
